...
function! DevhelpUpdate (flag, word)
 try
...
function! DevhelpUpdateN (flag)
	let s:word = GetCursorWord()
	call DevhelpUpdate (a:flag, s:word)
endfunction

function! DevhelpUpdateI (flag)
  " Use normal update function
 call DevhelpUpdateN (a:flag)
...
function! DevhelpUpdateV (flag)
	silent exe 'norm! gv""y'
	let s:word = getreg('"')
	echo s:word
	call DevhelpUpdate (a:flag, s:word)
endfunction
...
if exists ('g:devhelpSearch') && g:devhelpSearch
 ...
 exec 'nmap '.g:devhelpSearchKey.' :call DevhelpUpdateN("s")<CR>'
 exec 'imap '.g:devhelpSearchKey.' <ESC>:call DevhelpUpdateI("s")<CR>'
 exec 'vmap '.g:devhelpSearchKey.' :<C-U>call DevhelpUpdateV("s")<CR>'
endif
...
if exists ('g:devhelpAssistant') && g:devhelpAssistant
 ...
 au! CursorHold  * nested call DevhelpUpdateN('a')
 au! CursorHoldI * nested call DevhelpUpdateN('a')
endif
